...
public class LocalVpnService extends VpnService {
    ...
    private void startVpn() {
        ...
        try {
            ...
            Log.i(TAG, "VPN 已启动 - TCP转发 + DNS代理就绪");
        } catch (Exception e) {
    ...
    private void trafficLoop() {
        ...
        try {
            ...
            dnsSocket.setSoTimeout(100);

            while (!shouldStop && isRunning) {
                // 1. 处理 TCP 响应
                tcpForwarder.processResponses();
            ...
            if (tcpForwarder != null) tcpForwarder.stop();
        }
    }
    ...
    private void processPacket(byte[] pkt, FileOutputStream out, DatagramSocket dnsSocket) {
        ...
        } else if (protocol == 6 && pkt.length > ihl + 20) {
            // TCP - 通过 TcpForwarder 转发
            tcpForwarder.handlePacket(pkt, ihl, out);
        }
        // 其他协议忽略
    }
    ...
    private void writeDnsResponse(byte[] reqPkt, int ihl, byte[] dnsResp, int dnsRespLen,
        ...
        // UDP 头：交换端口
        int off = ihl;
        pkt[off] = reqPkt[off + 2]; pkt[off + 1] = reqPkt[off + 3];
        pkt[off + 2] = reqPkt[off]; pkt[off + 3] = reqPkt[off + 1];
